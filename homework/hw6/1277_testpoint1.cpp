...
void Subtask1(int testcase) {
    //subtask1: basic function
    try {
        static int ROUND_ADD = 40, ROUND_DEL = 10, ROUND_TOTAL = 30;
        Pokedex pokedex("pokedex");
        for (int i = 1; i <= ROUND_ADD; i++) {
            int id = (testcase - 1) * ROUND_TOTAL + i;
        ...
        for (int i = 1; i <= ROUND_TOTAL * (testcase - 1) + ROUND_ADD; i++) { //40*(n-1) + 50
            std::cout << pokedex.pokeFind(i * EXPAND) << '\n';
        }

        for (int i = ROUND_TOTAL * (testcase - 1) + ROUND_ADD - ROUND_DEL + 1; i <= ROUND_TOTAL * (testcase - 1) + ROUND_ADD; i++) {
            //std::cout << "delete " << i << '\n';
            std::cout << pokedex.pokeDel(i * EXPAND) << '\n'; // delete
        }

        for (int i = 1; i <= ROUND_TOTAL * (testcase - 1) + ROUND_ADD; i++) { //40*(n-1) + 50, Not Founded
            std::cout << pokedex.pokeFind(i * EXPAND) << '\n';
            for (int j = 0; j < 7; ++j) {
...
void Subtask2(int testcase) {
    //subtask2: iterator

    try {
        static int ROUND_ADD = 400, ROUND_DEL = 100, ROUND_TOTAL = 300;
        Pokedex pokedex("pokedex");
        for (int i = 1; i <= ROUND_ADD; i++) {
            int id = (testcase - 1) * ROUND_TOTAL + i;
            std::string name = encode(id);
